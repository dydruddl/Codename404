	...
	<div id="frame">
		...
		<div class="container">
			...
			<ul class="nav nav-tabs" role="tablist">
				...
				<li class="active"><a href="qna_sol.qna?qna_sol=1">해결</a></li>
			...
			<div class="tab-content" align=center>
						...
						<table class="table table-striped">
							...
							<c:if test="${not empty solist}">
								...
								<c:set var="snum" value="${solnum }"></c:set>
								<!-- 컨트롤러에서 정한 화면 출력번호no를 set태그로 설정 -->
								<c:forEach var="qnaBoard" items="${solist }">
									<c:if test="${qnaBoard.qna_sol == 1 }">
										<tr>
											...
											<c:if test="${qnaBoard.qna_del !=1 }">
												...
												<td><a
													href="qnaView.qna?qna_no=${qnaBoard.qna_no}&pageNum2=${pp2.currentPage2}"
					 ...
					 <div class="fl-search-container">
						<form action="qna_sol.qna" align="center">
							<!-- 검색 기능 폼-->
						...
						<ul class="pagination">
							<c:if test="${empty keyword}">
								<c:if test="${pp2.startPage2 > pp2.pagePerBlk2 }">
									<!-- startPage(1,11,21...) > pagePerBlk(1개 블록에 10개 페이지) -->
									<li><a href="qna_sol.qna?pageNum2=${pp2.startPage2 - 1}&qna_sol=1">이전</a></li>
								</c:if>
								<c:forEach var="i" begin="${pp2.startPage2}" end="${pp2.endPage2}">
									<!-- [1][2][3].. 10개씩 페이지 출력 -->
									<li <c:if test="${pp2.currentPage2==i}">class="active"</c:if>>
										<!-- 현재 페이지: 활성화 상태 --> <a href="qna_sol.qna?pageNum2=${i}&qna_sol=1">${i}</a>
									</li>
								</c:forEach>
								<c:if test="${pp2.endPage2 < pp2.totalPage2}">
									<!-- endPage(10,20,30...) < 총 페이지수 -->
									<li><a href="qna_sol.qna?pageNum2=${pp2.endPage2 + 1}&qna_sol=1">다음</a></li>
								</c:if>
							</c:if>
						</ul>
