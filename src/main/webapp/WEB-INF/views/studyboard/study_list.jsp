...
<ul class="nav nav-tabs">
	<li class="active"><a href="#study_list.study">전체</a></li>
...
<ul class="nav nav-tabs">
	...
	<li><a data-toggle="tab" href="#menu1">모집완료</a></li>
	<li><a data-toggle="tab" href="#menu2">모집중</a></li>
</ul>
 --%>
...
<%-- 글작성 버튼 --%>
	<!-- 로그인 상태 -->
    <c:if test="${empty sessionScope.id and empty adminmodel.admin_id}">
...
<table class="table table-striped">
	...
	<c:set var="no1" value="${no}"></c:set> <%-- var: 변수 설정, value="${받아온 값}" --%>
...
<!-- 
 ...
 -->			
	...
	<c:set var="no1" value="${no1 - 1}"/> <!-- 순차적으로 번호값 감소 -->
	</c:forEach>
	</c:if>
</table>
     
<div class="fl-search-container"> 
<form action="study_list.study" align="center"> <!-- 검색 기능 폼-->
...
<ul class="pagination">
	...
	<c:if test="${not empty keyword}">
		<c:if test="${pp.startPage > pp.pagePerBlk}"> <!--컨트롤러에서 갖고온 pp 객체 -->
			<li><a href="study_list.study?pageNum=${pp.startPage - 1}&search=${search}&keyword=${keyword}">이전</a></li>
		</c:if>
		
		<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
			<!-- [1][2][3]..  루프 돌려서 페이지 출력 -->
			<li <c:if test="${pp.currentPage==i}">class="active"</c:if>>
			<a href="study_list.study?pageNum=${i}&search=${search}&keyword=${keyword}">${i}</a></li>
			<!-- pageNum, search, keyword 가져가야됨!!!!!! -->
		</c:forEach>
		<c:if test="${pp.endPage < pp.totalPage}">
			<li><a href="study_list.study?pageNum=${pp.endPage + 1}&search=${search}&keyword=${keyword}">다음</a></li>
		</c:if>
	</c:if>
	...
	<c:if test="${empty keyword}">
		<c:if test="${pp.startPage > pp.pagePerBlk }">
			...
			<li><a href="study_list.study?pageNum=${pp.startPage - 1}">이전</a></li>
		</c:if>
		...
		<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
			...
			<li <c:if test="${pp.currentPage==i}">class="active"</c:if>>
			<!-- 현재 페이지: 활성화 상태 -->
			<a href="study_list.study?pageNum=${i}">${i}</a></li>
		</c:forEach>
		
		<c:if test="${pp.endPage < pp.totalPage}">
		<!-- endPage(10,20,30...) < 총 페이지수 -->
			<li><a href="study_list.study?pageNum=${pp.endPage + 1}">다음</a></li>
		</c:if>
	</c:if>
</ul>
