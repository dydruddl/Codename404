...
<ul class="nav nav-tabs">
	...
	<li class="active"><a href="study_mozip.study">모집완료</a></li> 
	<li><a href="study_moziping.study">모집중</a></li>
</ul>

<!------------------------------ content div 시작 / center로 해야 페이지블록이 센터로 옴 ------------------------------>
<div class="tab-content" align="center">
<!-------------------------------- 모집완료 눌렀을 때 ------------------------------>
<div id="study_mozip.study" class="tab-pane fade in active"> <!-- menu1 -->
...
<table class="table table-striped">
	...
	<c:if test="${empty mozipList}">
		<tr>
			<td colspan="7">데이터가 없습니다</td>
		</tr>
	</c:if>
    <c:if test="${not empty mozipList}">
	<c:set var="no2" value="${mozipNum}"></c:set> <%-- ${mozipNum}  var: 변수 설정, value="${받아온 값}" --%>
	    
	<!-- 컨트롤러에서 정한 화면 출력번호 no를 set태그로 설정 -->
	<c:forEach var="studyBoard" items="${mozipList}">
	...
	<c:if test="${studyBoard.study_mozip == 1}">
		<tr>
			...
			<c:if test="${studyBoard.study_del != 1}"> <!-- 살아있는 글 제목에 링크걸기 --> <!-- 글 번호값, 페이지번호값 가져가기 -->
				<td><a href="studyView.study?study_no=${studyBoard.study_no}&pageNum=${pp2.currentPage}" 
		...
		<c:set var="no2" value="${no2 - 1}"/> <!-- 순차적으로 번호값 감소 -->
...
<form action="study_mozip.study" align="center"> 
	<input type="hidden" name="pageNum" value="1">
	<!-- 페이지(1)와 search, keyword 가지고 list 로 이동 -->
...
<ul class="pagination">
	...
	<c:if test="${not empty keyword}">
		<c:if test="${pp2.startPage > pp2.pagePerBlk}"> <!--컨트롤러에서 갖고온 pp 객체 -->
			<li><a href="study_mozip.study?pageNum=${pp2.startPage - 1}&search=${search}&keyword=${keyword}">이전</a></li>
		</c:if>
		
		<c:forEach var="i" begin="${pp2.startPage}" end="${pp2.endPage}">
			<!-- [1][2][3]..  루프 돌려서 페이지 출력 -->
			<li <c:if test="${pp2.currentPage==i}">class="active"</c:if>>
			<a href="study_mozip.study?pageNum=${i}&search=${search}&keyword=${keyword}">${i}</a></li>
			<!-- pageNum, search, keyword 가져가야됨!!!!!! -->
		</c:forEach>
		<c:if test="${pp2.endPage < pp2.totalPage}">
			<li><a href="study_mozip.study?pageNum=${pp2.endPage + 1}&search=${search}&keyword=${keyword}">다음</a></li>
		</c:if>
	</c:if>
	
	<!-- 전체 목록의 페이징 처리: 글번호, 페이지 -->
	<c:if test="${empty keyword}">
		<c:if test="${pp2.startPage > pp2.pagePerBlk}">
			<!-- startPage(1,11,21...) > pagePerBlk(1개 블록에 10개 페이지) -->
			<li><a href="study_mozip.study?pageNum=${pp2.startPage - 1}">이전</a></li>
		</c:if>
		...
		<c:forEach var="i" begin="${pp2.startPage}" end="${pp2.endPage}">
			...
			<li <c:if test="${pp2.currentPage==i}">class="active"</c:if>>
			<!-- 현재 페이지: 활성화 상태 -->
			<a href="study_mozip.study?pageNum=${i}">${i}</a></li>
		</c:forEach>
		
		<c:if test="${pp2.endPage < pp2.totalPage}">
		<!-- endPage(10,20,30...) < 총 페이지수 -->
			<li><a href="study_mozip.study?pageNum=${pp2.endPage + 1}">다음</a></li>
		</c:if>
	</c:if>
</ul>
